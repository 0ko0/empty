...
local function RandomString()
    local str = ""
    for i = 1, math.random(10, 20) do str = str .. string.char(math.random(97, 122)) end
    return str
end

local SimpleSpy3 = Create("ScreenGui",{Name = RandomString(), ResetOnSpawn = false, DisplayOrder = 2147483647})
local Storage = Create("Folder",{Name = RandomString()})
...
local SearchBox = Create("TextBox",{Parent = LeftPanel,BackgroundColor3 = Color3.fromRGB(45, 45, 45),BorderSizePixel = 0,Position = UDim2.new(0, 5, 0, 5),Size = UDim2.new(1, -10, 0, 20),Font = Enum.Font.SourceSans,PlaceholderText = "Search Remotes...",Text = "",TextColor3 = Color3.new(1, 1, 1),TextSize = 14})
...
local Simple = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Color3.new(1, 1, 1),AutoButtonColor = false,BackgroundTransparency = 1,Position = UDim2.new(0, 5, 0, 0),Size = UDim2.new(0, 57, 0, 18),Font = Enum.Font.SourceSansBold,Text =  "SimpleSpy remake",TextColor3 = Color3.new(1, 1, 1),TextSize = 14,TextXAlignment = Enum.TextXAlignment.Left})
...
MakeDraggable(TopBar, Background)
...
function makeToolTip(enable, text)
    if enable and text then
        ...
        connections["ToolTip"] = RunService.RenderStepped:Connect(function()
            ...
            local yOffset = isMobile and -60 or -15 
            local topLeft = MousePos + Vector2.new(20, yOffset)
...
newButton("Spam (x100)", 
    function() return "spam remote" end,
    function()
        if selected and selected.Remote then
            TextLabel.Text = "✅️"
            task.spawn(function()
                for i = 1, 100 do
                    ...
                    elseif selected.Remote:IsA("RemoteFunction") then
                        task.spawn(function() selected.Remote:InvokeServer(unpack(selected.args)) end)
                    end
...
if UserInputService.TouchEnabled and not UserInputService.MouseEnabled then
    ...
    QuickCapture.Position = UDim2.new(0.5, -16, 0, 10) 
    ...
    MakeDraggable(QuickCapture, QuickCapture)
